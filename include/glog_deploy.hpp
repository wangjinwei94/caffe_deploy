...
#include <unistd.h>
#include <sys/time.h>
static inline int __GetTimeUS(void) noexcept(true) {
	timeval time;
	gettimeofday(&time, nullptr);
	return time.tv_usec;
}
#else  // defined(__linux__)
static inline int getpid(void) noexcept(true) {
	return 0;
}
static inline int __GetTimeUS(void) noexcept(true) {
	return 0;
...
#include <execinfo.h>
#include <cxxabi.h>
static inline std::string __Demangle(const std::string& name) noexcept(true) {
...
public:
	__GlogDeoloyNullStream() noexcept(true): std::ostream(nullptr) {
	}
	template <typename T>
	inline __GlogDeoloyNullStream& operator<<(T const & x) noexcept(true) {
		return *this;
	}
};
...
public:
	inline __GlogDeployLogMessage(const std::string& level, const std::string& file, const int line) noexcept(true):
	...
	// TODO: Jinwei: throw here is dangerous
	inline ~__GlogDeployLogMessage() noexcept(false) {
	...
	inline std::ostream& Stream(void) noexcept(true) {
		return *stream_;
	}
	static bool enable;
